...
function ApplicantProHelper() {}


  ApplicantProHelper.prototype.calculateStatus = function(account) {
    const now = moment();
  ...
  ApplicantProHelper.prototype.calculateSubStatus = function(account) {
    const now = moment();
    const planType = _.get(account, 'plan.type');
    ...
    } else if (planStatus === 'Cancelled' && _.includes(['Monthly', 'Annual'], planType) && !dateOfCancellation) {
...
module.exports = new ApplicantProHelper();